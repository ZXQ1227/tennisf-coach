...
| 1.1.4 | 2026-06-26 | **P0 个性化 + 首页 Today 模块**。训练推荐个性化：覆盖 14 种弱点、5 种目标的推荐计划映射，按 ntrpLevel 动态计算时长/难度/热量，技能格子按弱点→目标→默认→优势排序，弱点卡片荧光绿边框+「重点」角标，`techScores < 50` 自动标为重点，无档案时显示引导条。首页 Today 模块：已登录用户静态 CTA row 替换为智能 Today 卡（两状态：未打球展示个性化推荐训练，今日已打球展示成就/场地/连续天数），未登录保留原 CTA row。「打球记录」合并卡：训练日历与挥拍记录合并为单卡，挥拍摘要在上，细分割线，一周点亮日历在下。移除训练页「自定义训练」入口。首页昵称编辑图标放大 32→44rpx。 |
